<div class="main-content-new">
    <div class="list-pri wow fadeInUp delay02">
        <div class="row mt-2">
            <div class="col-md-6">
                <h3 class="title-pri"><?= lang('image') ?></h3>
            </div>
            <div class="col-md-6">
                <input type="text" id="datepicker-years" data-old="<?= $year ?? '1' ?>" value="<?= $year ?? '' ?>" class="datepicker-years search-year" placeholder="Năm">
            </div>
        </div>
        <div class="row">
            ...
            <?php if (!empty($data)) foreach ($data as $key => $value) { ?>
                ...
                <script>
                    album['<?= $value->id ?>'] = <?= convertWaterMark($value->album); ?>;
                </script>
            <?php } else { ?>
