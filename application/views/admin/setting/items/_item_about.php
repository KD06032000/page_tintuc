            ...
            <?php foreach ($this->config->item('cms_language') as $lang_code => $lang_name) { ?>
                <div class="tab-pane <?php echo ($lang_code == 'vi') ? 'active' : ''; ?>"
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                <?= form_input($meta_key . "[" . $meta_key . $id . "][$lang_code][subtitle]",
                                    !empty($item[$lang_code]['subtitle']) ? $item[$lang_code]['subtitle'] : ''
                                    , ['class' => 'form-control', 'placeholder' => 'Tiêu đề phụ']) ?>
                            </div>
                        </div>
                        <div class="col-md-12">
                            <?= form_textarea($meta_key . "[" . $meta_key . $id . "][$lang_code][desc]",
                                !empty($item[$lang_code]['desc']) ? $item[$lang_code]['desc'] : ''
                                , ['class' => 'form-control tinymce', 'row' => 8, 'placeholder' => 'Mô tả']) ?>
                        </div>
                    </div>
                </div>
            <?php } ?>
